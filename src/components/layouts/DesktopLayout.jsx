          ...
          <div className="flex items-center gap-4">
            {user ? (
              <>
                ...
                {/* Mobile Navigation Controls (Included in DesktopLayout for resizing support) */}
      ...
      <AnimatePresence>
        {searchOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-black/20 backdrop-blur-sm flex items-start justify-center pt-24 px-4"
          ...
          >
            <motion.div
              ...
              className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col max-h-[70vh]"
            >
              <div className="p-4 border-b border-gray-100 flex items-center gap-3">
                <Search size={20} className="text-gray-400" />
                <input
                  type="text"
                  placeholder="Search quizzes, notes, or people..."
                ...
                <button
                  onClick={() => setSearchOpen(false)}
                  className="px-3 py-1 bg-gray-100 text-xs font-bold text-gray-500 rounded-md hover:bg-gray-200"
                >
                  ESC
                </button>
              </div>
              ...
              <div className="overflow-y-auto p-2">
                {!searchQuery ? (
                  <div className="p-12 text-center text-gray-400 text-sm">
                    Type to start searching across the entire app.
                  </div>
                ) : (
                  <>
                    {filteredQuizzes.length > 0 ? (
                      <div className="space-y-1">
                        <h3 className="px-4 py-2 text-xs font-bold text-gray-400 uppercase">Quizzes</h3>
                        {filteredQuizzes.map(quiz => (
                          ...
                          >
                            <div className={`p-3 rounded-lg ${quiz.color ? quiz.color.replace('text-', 'bg-').replace('600', '100') : 'bg-gray-100'} ${quiz.color || 'text-gray-600'}`}>
                              {quiz.icon ? <quiz.icon size={20} /> : <BookOpen size={20} />}
                            </div>
                            <div>
                              <h4 className="font-bold text-gray-900 group-hover:text-black">{quiz.title}</h4>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                          ...
                          {(showAllTopics ? allQuizzes : allQuizzes.slice(0, 3)).map((quiz) => (
                        ...
                        {!showAllTopics && allQuizzes.length > 3 && (
                          <div className="mt-12 flex justify-center">
                            <motion.button
                              whileHover={{ scale: 1.05, y: -2 }}
                              whileTap={{ scale: 0.95 }}
                              onClick={() => setShowAllTopics(true)}
                              className="px-10 py-4 bg-black text-white rounded-2xl font-bold flex items-center gap-3 shadow-xl hover:shadow-2xl transition-all"
                            >
